...
public void OnPluginStart()
{
	...
	*/
	...
	g_hCvarChance1 = CreateConVar(		"l4d2_gift_rewards_chance_ammo",	"50",			"Chance to completely refill a players ammo.", CVAR_FLAGS );
	g_hCvarChance2 = CreateConVar(		"l4d2_gift_rewards_chance_heal",	"20",			"Chance to restore a players health to full.", CVAR_FLAGS );
	g_hCvarChance3 = CreateConVar(		"l4d2_gift_rewards_chance_speed",	"30",			"Chance to reward increased speed for: shooting, reloading and deploying. Requires WeaponHandling API plugin.", CVAR_FLAGS );
...
void GiveAward(int client)
{
	if( GetClientTeam(client) == 2 )
	{
		...
		if( random )
		{
			...
			PrintToChatAll("\x04[\x05Gift\x04] \x01奖励 \x05%s \x01给 \x04%N", random == 1 ? "弹药" : random == 2 ? "生命值" : "加速度", client);
		}
	}
}
