...
void IsUpdateEntCount(int client, int g_Multiple, bool g_bPrompt, bool g_bContent, bool g_bMoreCheck, int g_iClientNumber, int g_iSurvivorLimit)
{
	...
	if(g_iMoreMedical & SurvivorsPrompt && g_bMoreCheck)
	{
		if(g_bPrompt)
			if(!g_bPlayerPrompt)
				PrintToChatAll("\x04[HspAF]\x03%N\x05%s\x04(\x03%i\x05/\x03%d\x04)\x03...\x04%s", client, g_bContent ? "正在连接" : "断开连接", g_iPlayerNumber, g_iSurvivorLimit, g_bContent ? "(づˉ*ˉ)づ" : "╮(๑•́ ₃•̀๑)╭");
			else
				PrintToChatAll("\x04[HspAF]\x03%N\x05%s\x04(\x03%i\x05/\x03%d\x04)\x03,\x05变更为\x03%s\x05倍医疗物品.", client, g_bContent ? "正在连接" : "断开连接", g_iPlayerNumber, g_iSurvivorLimit, g_sMedical);
		else
			PrintToChatAll("\x04[HspAF]\x05当前联结人数为\x03:\x04(\x03%i\x05/\x03%d\x04)\x03,\x05变更为\x03%s\x05倍医疗物品.", g_iClientNumber, g_iSurvivorLimit, g_sMedical);
	}
}
