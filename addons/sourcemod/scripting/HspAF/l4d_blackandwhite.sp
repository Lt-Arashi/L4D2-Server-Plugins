...
public void OnPluginStart()
{
	...
	h_cvarNoticeType = CreateConVar("l4d_bandw_notice", "1", "0=关闭提示, 1=仅提示生还, 2=全部提示, 3=仅提示感染.", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	h_cvarPrintType = CreateConVar("l4d_bandw_type", "1", "0=聊天框提示, 1=hint弹出提示, 2=两个一起提示.", FCVAR_NOTIFY, true, 0.0, true, 2.0);
		...
		else if(bandw_notice == 2) 
		{
			if(IsFakeClient(target))
			{
				if(bandw_type == 1 || bandw_type == 2) 
					if(!idleplayer)
						PrintHintTextToAll("%N已经在看黑白电视了", target);
					else
						PrintHintTextToAll("闲置:%N(%N)已经在看黑白电视了", idleplayer, target);
				if(bandw_type == 0 || bandw_type == 2)
					if(!idleplayer)
						CPrintToChatAll("{default}[{green}!{default}] {blue}%N {olive}已经在看黑白电视了", target);
					else
						CPrintToChatAll("{default}[{green}!{default}] {blue}闲置:%N{green}(%N) {olive}已经在看黑白电视了", idleplayer, target);
			}
			else
			{
				if(bandw_type == 1 || bandw_type == 2) 
					PrintHintTextToAll("%N(%s)已经在看黑白电视了", target, GetBotName(target));
				if(bandw_type == 0 || bandw_type == 2)
					CPrintToChatAll("{default}[{green}!{default}] {blue}%N{green}(%s) {olive}已经在看黑白电视了", target, GetBotName(target));
			}
		}
		//print to infected
		else if(bandw_notice == 3)
		{
			for( int x = 1; x <= MaxClients; x++)
			{
				...
				{	
					if(bandw_type == 1 || bandw_type == 2) 
						if(!idleplayer)
							PrintHintText(x, "%N已经在看黑白电视了", target);
						else
							PrintHintText(x, "闲置:%N(%N)已经在看黑白电视了", idleplayer, target);
					if(bandw_type == 0 || bandw_type == 2)
						if(!idleplayer)
							CPrintToChat(x, "{default}[{green}!{default}] {blue}%N {olive}已经在看黑白电视了", target);
						else
							CPrintToChat(x, "{default}[{green}!{default}] {blue}闲置:%N{green}(%N) {olive}已经在看黑白电视了", idleplayer, target);
				}
				else
				{
					if(bandw_type == 1 || bandw_type == 2)
						PrintHintText(x, "%N(%s)已经在看黑白电视了", target, GetBotName(target));
					if(bandw_type == 0 || bandw_type == 2)
						CPrintToChat(x, "{default}[{green}!{default}] {blue}%N{green}(%s) {olive}已经在看黑白电视了", target, GetBotName(target));	
				}
			}
		}
		//print to survivors
	...
	if(g_bLastlife[target])
	{
		...
		else if(bandw_notice == 2) 
		{
			if(IsFakeClient(target))
			{
				if(bandw_type == 1 || bandw_type == 2) 
					if(!idleplayer)
						PrintHintTextToAll("%N接受了治疗，世界再次有了颜色", target);
					else
						PrintHintTextToAll("闲置:%N(%N)接受了治疗，世界再次有了颜色", idleplayer, target);
				if(bandw_type == 0 || bandw_type == 2)
					if(!idleplayer)
						CPrintToChatAll("{default}[{green}!{default}] {blue}%N {olive}接受了治疗，世界再次有了颜色", target);
					else
						CPrintToChatAll("{default}[{green}!{default}] {blue}闲置:%N{green}(%N) {olive}接受了治疗，世界再次有了颜色", idleplayer, target);
			}
			else
			{
				if(bandw_type == 1 || bandw_type == 2) 
					PrintHintTextToAll("%N(%s)接受了治疗，世界再次有了颜色", target, GetBotName(target));
				if(bandw_type == 0 || bandw_type == 2)
					CPrintToChatAll("{default}[{green}!{default}] {blue}%N{green}(%s) {olive}接受了治疗，世界再次有了颜色", target, GetBotName(target));
			}
		}
		//print to infected
		else if(bandw_notice == 3)
		{
			for( int x = 1; x <= MaxClients; x++)
			{
				...
				{	
					if(bandw_type == 1 || bandw_type == 2) 
						if(!idleplayer)
							PrintHintText(x, "%N接受了治疗，世界再次有了颜色", target);
						else
							PrintHintText(x, "闲置:%N(%N)接受了治疗，世界再次有了颜色", idleplayer, target);
					if(bandw_type == 0 || bandw_type == 2)
						if(!idleplayer)
							CPrintToChat(x, "{default}[{green}!{default}] {blue}%N {olive}接受了治疗，世界再次有了颜色", target);
						else
							CPrintToChat(x, "{default}[{green}!{default}] {blue}闲置:%N{green}(%N) {olive}接受了治疗，世界再次有了颜色", idleplayer, target);
				}
				else
				{
					if(bandw_type == 1 || bandw_type == 2) 
						PrintHintText(x, "%N(%s)接受了治疗，世界再次有了颜色", target, GetBotName(target));
					if(bandw_type == 0 || bandw_type == 2)
						CPrintToChat(x, "{default}[{green}!{default}] {blue}%N{green}(%s) {olive}接受了治疗，世界再次有了颜色", target, GetBotName(target));	
				}
			}
		}
		//print to survivors
