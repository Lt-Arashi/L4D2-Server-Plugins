...
public void OnPluginStart()
{
	...
	g_hWhichTeamToShow = CreateConVar("mvp_witch_team_show", "2", "允许给哪个团队显示 MVP 信息 (0: 所有团队, 1: 仅旁观者团队, 2: 仅生还者团队, 3: 仅特感团队)", CVAR_FLAG, true, 0.0, true, 3.0);
	...
	g_hAllowShowCi = CreateConVar("mvp_allow_show_ci", "0", "是否允许显示丧尸击杀信息", CVAR_FLAG, true, 0.0, true, 1.0);
	...
	g_hAllowShowRankCI = CreateConVar("mvp_show_your_rank_ci", "0", "是否允许显示你的丧尸排名", CVAR_FLAG, true, 0.0, true, 1.0);
...
public void missionLostHandler(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (g_hAllowShowFailCount.BoolValue) {
		CPrintToChatAll("{blue}[{default}HspAF{blue}]: {default}这是你们第 {olive}%d {default}次团灭，你们的技术简直是一坨屎 (*･ω< )", ++failCount);
	}
