...
{
	// CreateConVars
	g_hAllowBhop = CreateConVar("ai_ChargerBhop", "0", "是否开启 Charger 连跳", CVAR_FLAG, true, 0.0, true, 1.0);
	...
	g_hExtraTargetDist = CreateConVar("ai_ChargerExtraTargetDistance", "200,400", "Charger 会在这一范围内寻找其他有效的目标（中间用逗号隔开，不要有空格）", CVAR_FLAG);
	g_hAimOffset = CreateConVar("ai_ChargerAimOffset", "60.0", "目标的瞄准水平与 Charger 处在这一范围内，Charger 不会冲锋", CVAR_FLAG, true, 0.0);
