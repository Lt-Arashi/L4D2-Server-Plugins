...
public Action l4d2_timer_kick_switch(int client, int args)
{
	if(bCheckClientAccess(client))
	{
		switch(g_Kickafk)
		{
			case 0:
			{
				PrintToChat(client, "\x04[Prompt]\x05普通玩家闲置超时踢出已禁用,请在CFG中设为1启用.");
			}
			case 1,2:
			{
				if(l4d2_timer_kick)
				{
					...
					if(g_Kickafk == 1)
						PrintToChatAll("\x04[Prompt]\x03已关闭\x05普通玩家闲置超时踢出.");
					else if(g_Kickafk == 2)
						PrintToChatAll("\x04[Prompt]\x03已关闭\x05普通玩家和管理员闲置超时踢出.");
				}
				else
				{
					forkickafkplayer();
					...
					if(g_Kickafk == 1)
						PrintToChatAll("\x04[Prompt]\x03已开启\x05普通玩家闲置超时踢出.");
					else if(g_Kickafk == 2)
						PrintToChatAll("\x04[Prompt]\x03已开启\x05普通玩家和管理员闲置超时踢出.");
				}
			}
		}
	}
	else
		PrintToChat(client, "\x04[Prompt]\x05你无权使用此指令.");
			...
			case 1:
			{
				...
				else
				{
					if (KickLookOnPlayer[client] >= g_kickplayer)
					{
						...
						PrintToChatAll("\x04[Prompt]\x03%N\x05挂机超过\x03%s\x05已被服务器踢出.", client, sRunTime);//聊天窗提示
						kickPlayerTimer[client] = null;
						KickLookOnPlayer[client] = 0;
						return Plugin_Stop;
					}
					else
					{
						...
						PrintHintText(client, "你有 %s 的时间解决要事后返回游戏.", sRunTime);//屏幕中下提示.
					}
