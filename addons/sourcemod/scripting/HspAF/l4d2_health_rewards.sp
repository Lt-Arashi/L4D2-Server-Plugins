	...
	if(g_iDefibrillator != 0 && IsValidClient(client) && GetClientTeam(client) == 2)
	{
		if(IsValidClient(subject) && GetClientTeam(subject) == 2)
		{
			if(client != subject)
			{
				...
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05救活了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(client), GetTrueName(subject), GetRewardHealth(iReward));
				}
				else
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05救活了\x03%s\x04,\x05血量已达\x03%d\x05上限.", GetTrueName(client), GetTrueName(subject), g_iLimitHealth);//聊天窗提示.
				}
			}
		}
	}
	...
	if(g_iReviveSuccess != 0 && IsValidClient(client) && GetClientTeam(client) == 2)
	{
		if(IsValidClient(subject) && GetClientTeam(subject) == 2)
		{
			if(client != subject)
			{
				...
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05拉起了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(client), GetTrueName(subject), GetRewardHealth(iReward));
				}
				else
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05拉起了\x03%s\x04,\x05血量已达\x03%d\x05上限.", GetTrueName(client), GetTrueName(subject), g_iLimitHealth);//聊天窗提示.
				}
			}
		}
	}
	...
	if(g_iHealSuccess != 0 && IsValidClient(client) && GetClientTeam(client) == 2)
	{
		if(IsValidClient(subject) && GetClientTeam(subject) == 2)
		{
			if(client != subject)
			{
				...
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05治疗了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(client), GetTrueName(subject), GetRewardHealth(iReward));
				}
				else
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05治疗了\x03%s\x04,\x05血量已达\x03%d\x05上限.", GetTrueName(client), GetTrueName(subject), g_iLimitHealth);//聊天窗提示.
				}
			}
		}
	}
	...
	if(g_iSurvivorRescued != 0 && IsValidClient(client) && GetClientTeam(client) == 2)
	{
		if(IsValidClient(rescuer) && GetClientTeam(rescuer) == 2)
		{
			if(client != rescuer)
			{
				...
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05营救了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(rescuer), GetTrueName(client), GetRewardHealth(iReward));
				}
				else
				{
					if(iReward > 0)
						PrintToChatAll("\x04[HspAF]\x03%s\x05营救了\x03%s\x04,\x05血量已达\x03%d\x05上限.", GetTrueName(rescuer), GetTrueName(client), g_iLimitHealth);//聊天窗提示.
				}
			}
		}
	}
...
public void Event_WitchHarasserSet(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidClient(client) && GetClientTeam(client) == 2)
		PrintToChatAll("\x04[HspAF]\x03%s\x05惊扰了\x03%s.", GetTrueName(client), GetWitchName(iWitchid));//聊天窗提示.
}
...
public void Event_Witchkilled(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(g_bWitchNumber == true && IsValidClient(client) && GetClientTeam(client) == 2)
	{
		...
		{
			if(iReward > 0)
				PrintToChatAll("\x04[HspAF]\x03%s\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", GetTrueName(client), sType, GetWitchName(iWitchid), GetRewardHealth(iReward));
		}
		else
		{
			if(iReward > 0)
				PrintToChatAll("\x04[HspAF]\x03%s\x05%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", GetTrueName(client), sType, GetWitchName(iWitchid), g_iLimitHealth);//聊天窗提示.
		}
	}
}
					...
					if(iReward > 0)
						if(iBot != 0)
						{
							if(IsValidClient(iBot))
								PrintToChat(iBot, "\x04[HspAF]\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", sType, GetPlayerName(client, iHLZClass), GetRewardHealth(iReward));
						}
						else
							PrintToChat(attacker, "\x04[HspAF]\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", sType, GetPlayerName(client, iHLZClass), GetRewardHealth(iReward));
				}
				else
				{
					if(iReward > 0)
						...
						{
							if(IsValidClient(iBot))
								PrintToChat(iBot, "\x04[HspAF]\x05%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", sType, GetPlayerName(client, iHLZClass), g_iLimitHealth);//聊天窗提示.
						}
						else
							PrintToChat(attacker, "\x04[HspAF]\x05%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", sType, GetPlayerName(client, iHLZClass), g_iLimitHealth);//聊天窗提示.
				}	
			}
			else
			{
				if(iReward > 0)
					...
					{
						if(IsValidClient(iBot))
							PrintToChat(iBot, "\x04[HspAF]\x05%s了\x03%s\x04.", sType, GetPlayerName(client, iHLZClass));//聊天窗提示.
					}
					else
						PrintToChat(attacker, "\x04[HspAF]\x05%s了\x03%s\x04.", sType, GetPlayerName(client, iHLZClass));//聊天窗提示.
