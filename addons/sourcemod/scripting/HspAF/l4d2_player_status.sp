...
public void Event_PlayerLedgeGrab(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidClient(client) && GetClientTeam(client) == 2)
		CPrintToChatAll("{default}[{green}!{default}] {blue}%s {olive}脚底抹油，挂边了", GetTrueName(client));//聊天窗提示.
}
...
void OnTakeDamageAlivePost(int victim, int attacker, int inflictor, float damage, int damagetype) 
{
	...
	if (IsValidClient(attacker)) 
	{
		...
		{
			case 2: 
				CPrintToChatAll("{default}[{green}!{default}] {olive}%s {default}用枪制服了 {blue}%s", GetTrueName(attacker), GetTrueName(victim));//聊天窗提示.
			case 3: 
			{
	...
	else
	{
		if (IsValidEntity(attacker)) 
		{
			...
			if (damagetype & DMG_DROWN && GetEntProp(victim, Prop_Data, "m_nWaterLevel") > 1)
				...
				CPrintToChatAll("{default}[{green}!{default}] {blue}%s {olive}后脑着地，摔死了", GetTrueName(victim));//聊天窗提示.
			else if (strcmp(classname, "worldspawn") == 0 && damagetype == 131072)
				CPrintToChatAll("{default}[{green}!{default}] {blue}%s {olive}大出血而死", GetTrueName(victim));//聊天窗提示.
				...
				CPrintToChatAll("{default}[{green}!{default}] {olive}烫脚哥 {blue}%s {default}已死亡", GetTrueName(victim));//聊天窗提示.
			else
				CPrintToChatAll("{default}[{green}!{default}] {blue}%s {olive}嘎了", GetTrueName(victim));//聊天窗提示.
		}
	}
}
...
public void Event_Incapacitate(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidClient(client) && GetClientTeam(client) == 2)
	{
		if (IsValidClient(attacker))
		{
			...
			{
				case 2: 
					CPrintToChatAll("{default}[{green}!{default}] {olive}%s {default}用枪制服了 {blue}%s", GetTrueName(attacker), GetTrueName(client));//聊天窗提示.
				case 3: 
				{
					int zombie_class = GetEntProp(attacker, Prop_Send, "m_zombieClass");
					if(zombie_class >= 1 && zombie_class <= 6)
		...
		else
		{
			if (IsValidEntity(entity)) 
			{
				...
				if (damagetype & DMG_DROWN && GetEntProp(client, Prop_Data, "m_nWaterLevel") > 1)
					...
					CPrintToChatAll("{default}[{green}!{default}] {blue}%s {olive}一屁股摔在地上起不来了", GetTrueName(client));//聊天窗提示.
				else if (strcmp(classname, "infected") == 0)
...
public Action Kill_Player(int client, int args)
{
	if(IsAliveSurvivor(client))
	{
		...
		CPrintToChatAll("{default}[{green}!{default}] {olive}生还者 {blue}%N {olive}一口气没上来，挂了", client);
	}
	else if(IsAliveInfected(client))
	{
		...
		CPrintToChat(client, "{default}[{green}!{default}] {olive}这个状态你自杀个寄吧");
